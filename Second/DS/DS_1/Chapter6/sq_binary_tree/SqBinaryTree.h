...
//  顺序二叉树的函数成员:
	...
	int Ancestor(int p, int q, bool flag=false) const;					// 求叉树中结点p和q的共同祖先
...
template <class ElemType>
SqBinaryTree<ElemType>::CreateSqBinaryTree(int size){
    maxSize = size;
...
template <class ElemType>
int SqBinaryTree<ElemType>::Ancestor(int p, int q, bool flag) const
// 操作结果：返回二叉树结点p和q的共同祖先
{
	...
	if(p > q){
		return Ancestor((p - 1) / 2, q, true);
	}
	else if(p < q){
		return Ancestor(p, (q - 1) / 2, true);
	}
	else{
		return p;
	}
